...
def celery_worker_init_db(**_):
    ...
    This ensures each Celery worker has its own dedicated connection to the MySQL/Postgresql database. 
    Otherwise one worker may close the connection while another worker is using it, raising exceptions.

    Without this, the existing session to the MySQL/Postgresql server is cloned to all Celery workers, so they
    all share a single session. A SQLAlchemy session is not thread/concurrency-safe, causing weird exceptions 
    to be raised by workers.
