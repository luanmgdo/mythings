...
class HardCoded(object):
    ...
    #DB_MODELS_IMPORTS = ('webgm',)  # Like CELERY_IMPORTS in CeleryConfig.
    ENVIRONMENT = property(lambda self: self.__class__.__name__)
    MAIL_EXCEPTION_THROTTLE = 24 * 60 * 60
    _SQLALCHEMY_DATABASE_DATABASE = 'webgm_softway'
    _SQLALCHEMY_DATABASE_HOSTNAME = 'localhost'
    _SQLALCHEMY_DATABASE_HOSTPORT = '5432'
    _SQLALCHEMY_DATABASE_PASSWORD = 'senhacomum'
    _SQLALCHEMY_DATABASE_USERNAME = 'luan_db_user'
...
class Config(HardCoded):
    ...
    SQLALCHEMY_DATABASE_URI = property(lambda self: 'postgresql://{u}:{p}@{h}:{d}/{b}'.format(
        b=quote_plus(self._SQLALCHEMY_DATABASE_DATABASE), d=quote_plus(self._SQLALCHEMY_DATABASE_HOSTPORT),
        h=quote_plus(self._SQLALCHEMY_DATABASE_HOSTNAME), p=quote_plus(self._SQLALCHEMY_DATABASE_PASSWORD), 
        u=quote_plus(self._SQLALCHEMY_DATABASE_USERNAME)
    ))
